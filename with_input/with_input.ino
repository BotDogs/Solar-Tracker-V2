...
float values[digits];   // array holding values for all the fields
int dp=0;
...
void input(){
  while(Serial.available()){
    ...
    if(num >= '0' && num <= '9' && dp==0){
      values[Ind] = ((values[Ind]*10) + (num - '0')); 
    }
    else if(num >= '0' && num <= '9'){
      values[Ind]+=static_cast<float>(num - '0')/pow(10,dp);
      dp++;
    }
    else if(num=='.'){
      dp++;
      ...
      if(values[1]<45){values[1]=45;}
      if(values[1]>135){values[1]=135;}
      TVx = cos(values[0]*M_PI/180.0)*cos(values[1]*M_PI/180.0), TVy = sin(values[0]*M_PI/180.0)*cos(values[1]*M_PI/180.0), TVz = sin(values[1]*M_PI/180.0);
      for(int i=0; i <= Ind; i++)
      {
...
void loop(){
  ...
  S2.runSpeed();
  int i=0;
  Serial.println((pow(pow(Nx[i] - CPx[i], 2) + pow(Ny[i] - CPy[i], 2) + pow(Nz[i] - CPz[i], 2), .5) - (pow(pow(Ix[i] - CPx[i], 2) + pow(Iy[i] - CPy[i], 2) + pow(Iz[i] - CPz[i], 2), .5) - dir1*spoolradius*static_cast<float>(S1.currentPosition())*M_PI/100.0)));
